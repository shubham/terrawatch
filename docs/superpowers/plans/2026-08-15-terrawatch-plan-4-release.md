...
### Task 4: Android platform correctness — SDK 36/37+, predictive back, adaptive layout, permissions [USER ADDITIONS 2026-08-15]
(a) **SDK 36 audit + Android 17 (API 37) forward-compat**: edge-to-edge sweep all screens (insets on every top/bottom bar), manifest audit vs 36/37 behavior changes list, targetSdk stays 36 for release (bump to 37 only when stable), document any 37-preview deltas that will bite. (b) **Predictive back**: `android:enableOnBackInvokedCallback=true`, verify every screen/back-stack hop animates predictively (nav-compose 2.9 support), detail-sheet + onboarding back behavior correct; emulator API-36 smoke for back-gesture allowed (disclosed exception — non-map screens; device gesture pass too). (c) **Adaptive layout (Android surfaces)**: replace raw-width `BoxWithConstraints` breakpoints with `material3-adaptive` WindowSizeClass (fixes the 900-980dp dead zone properly); verify compact/medium/expanded on device (portrait/landscape) + resizable-emulator foldable posture smoke (disclosed exception). (d) **Permission correctness**: kill the launch-time location ask (plan-3 backlog item) — location asked ONLY in onboarding step 2 / Settings "Use my location"; POST_NOTIFICATIONS only via Task 3's in-context ask; both with rationale handling (shouldShowRequestPermissionRationale → explainer before re-ask), permanent-denial path → Settings deep-link row, TDD'd permission-state reducer; device matrix: grant/deny/deny-permanently/revoke-mid-session for both permissions.

### Task 4b: Share targets (external item 5)
Share row in DetailSheet: WhatsApp/X/Threads package-targeted intents + availability check (PackageManager), fallback chooser; TDD intent-builder pure fn; device verify per-app.
